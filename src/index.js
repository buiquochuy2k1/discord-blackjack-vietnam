...
module.exports = async (message, options) => {
    ...
    if (commandType === "interaction" && !message.deferred && !message.replied) {
        ...
        starterMessage = await message.channel.send({ embeds: [{ title: "Đang tải tài nguyên", description: "Sòng sắp được mở, hãy chuẩn bị!" }] })
    }

    // check if the user is playing a game
    if (games.has(message.member.id)) {
        if (commandType === "message") {
            message.reply("Vui lòng hoàn thành ván đấu trước đó!")
        } else if (commandType === "interaction") {
            if (message.replied || message.deferred) {
                message.followUp({ content: "Vui lòng hoàn thành ván đấu trước đó!" })
            } else {
                message.reply({ content: "Vui lòng hoàn thành ván đấu trước đó!" })
            }
        }
        return {
            result: "None",
    ...
    let hitbtn = { label: "Bốc tiếp", style: 1, custom_id: "discord-blackjack-hitbtn", type: 2 }
    let standbtn = { label: "Bỏ bài", style: 1, custom_id: "discord-blackjack-standbtn", type: 2 }
    let ddownbtn = { label: "Nhân đôi cược", style: 1, custom_id: "discord-blackjack-ddownbtn", type: 2 }
    let splitbtn = { label: "Tách bài", style: 1, custom_id: "discord-blackjack-splitbtn", type: 2 }
    let cancelbtn = { label: "Huỷ ván đấu", style: 4, custom_id: "discord-blackjack-cancelbtn", type: 2 }
    ...
    // set the embeds
    let winEmbed = { title: "Bạn đã thắng!", color: 0x008800, description: "", fields: [], author: { name: message.member.user.tag, icon_url: message.member.user.displayAvatarURL() } }
    let loseEmbed = { title: "Bạn thua rồi!", color: 0xFF0000, description: "", fields: [], author: { name: message.member.user.tag, icon_url: message.member.user.displayAvatarURL() } }
    let tieEmbed = { title: "Hoà nhé", color: 0xFFFF00, description: "", fields: [], author: { name: message.member.user.tag, icon_url: message.member.user.displayAvatarURL() } }
    let timeoutEmbed = { title: "Hết thời gian!", color: 0xFF0000, description: "Quá thời gian chờ đợi (30 giây). Ván đấu sẽ được huỷ.", fields: [], author: { name: message.member.user.tag, icon_url: message.member.displayAvatarURL() } } 
    let cancelEmbed = { title: "Ván đấu kết thúc", color: 0xFF0000, description: "Bạn đã huỷ ván đấu. Hãy quay lại sau nhé.", fields: [], author: { name: message.member.user.tag, icon_url: message.member.displayAvatarURL() } }
    let generalEmbed = normalEmbed === false ? options.normalEmbedContent : { color: Math.floor(Math.random() * (0xffffff + 1)), fields: [{ name: "Bài của bạn", value: "", inline: true }, { name: `Bài của ${message.client.user.username}`, value: "", inline: true }], author: { name: message.member.user.tag, icon_url: message.member.user.displayAvatarURL() } }
    ...
    generalEmbed.fields[0].value = `Lá bài: ${yourcards.map(c => `[\`${c.emoji} ${c.rank}\`](https://google.com)`).join(" ")}\nTổng cộng:${isSoft ? " Bài mềm" : ""} ${yourcards.map(c => c.value).reduce((a, b) => b+a)}`
    generalEmbed.fields[1].value = `Lá bài: [\`${dealercards[0].emoji} ${dealercards[0].rank}\`](https://google.com) \` ? \`\nTổng cộng: \` ? \``
    ...
    if (yourcards.map(c => c.value).reduce((a, b) => b+a) === 21) {
        if (options.resultEmbed === true) {
            winEmbed.description = "Bạn đã được xì dách"
            winEmbed.fields.push({ name: "Bài của bạn", value: `Lá bài: [\`${yourcards[0].emoji} ${yourcards[0].rank}\`](https://google.com) [\`${yourcards[1].emoji} ${yourcards[1].rank}\`](https://google.com)\nTổng cộng: 21` })
            winEmbed.fields.push({ name: `Bài của ${message.client.user.username}`, value: `Lá bài: [\`${dealercards[0].emoji} ${dealercards[0].rank}\`](https://google.com)\nTổng cộng: \` ? \`` })
            commandType === "message" ? message.channel.send({ embeds: [winEmbed] }) : message.channel.send({ embeds: [winEmbed] })
        }

        return {
            result: "WIN",
            method: "Bạn đã được xì dách",
            ycard: yourcards,
            dcard: dealercards
        }
    }
    ...
    if (options.resultEmbed === true) {
        ...
        finalEmbed.fields.push({ name: `Bài của bạn`, value: `Lá bài: ${finalResult.ycard.map(c => `[\`${c.emoji} ${c.rank}\`](https://google.com)`).join(" ")}\nTổng cộng: ${finalResult.ycard.map(card => card.value).reduce((a, b) => b+a)}`, inline: true })
        finalEmbed.fields.push({ name: `Bài của ${message.client.user.username}`, value: `Lá bài: ${finalResult.dcard.map(c => `[\`${c.emoji} ${c.rank}\`](https://google.com)`).join(" ")}\nTổng cộng: ${finalResult.dcard.map(card => card.rank === "A" ? 1 : card.value).reduce((a, b) => b+a)}`, inline: true })
