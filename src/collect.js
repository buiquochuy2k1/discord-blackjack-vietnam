class Collect {
    ...
    async hit(message, userId, yourcard, dealercard, DECK, options) {
        ...
        if (options.isSplit === true && options.secondHand) {
            embed.description = "Bài của tay bên trái."
        } else if (options.secondHand) {
            embed.description = "Bài của tay bên phải"
        } else {
            embed.description = embed.description
        }
        embed.fields[0].value = `Lá bài: ${yourcard.map(c => `[\`${c.emoji} ${c.rank}\`](https://google.com)`).join(" ")}\nTổng cộng:${isSoft ? " Bài mềm" : ""} ${yourcard.map(c => c.value).reduce((a, b) => b + a)}`
        options.embed = embed
    ...
    async stand(message, userId, yourcard, dealercard, DECK, options) {
        ...
        if (yourvalue > 21) {
            finalResult = { result: "LOSE", method: "Bạn đã thua trong ván đấu này", ycard: yourcard, dcard: dealercard }
        } else if (yourvalue === 21) {
            finalResult = { result: "WIN", method: "Bạn đã được xì dách", ycard: yourcard, dcard: dealercard }
        } else if (yourvalue < 21 && dealervalue < yourvalue) {
            finalResult = { result: "WIN", method: "Bài của bạn thật tuyệt", ycard: yourcard, dcard: dealercard }
        } else if (dealervalue > yourvalue && dealervalue > 21 && yourvalue < 21) {
            finalResult = { result: "WIN", method: "Còn gì may mắn hơn không ?", ycard: yourcard, dcard: dealercard }
        } else if (dealervalue === 21 && yourvalue < 21) {
            finalResult = { result: "LOSE", method: "Nhà cái đã được xì dách", ycard: yourcard, dcard: dealercard }
        } else if (dealervalue > yourvalue && dealervalue < 21) {
            finalResult = { result: "LOSE", method: "Bài của nhà cái ngon hơn nyc của bạn =)))", ycard: yourcard, dcard: dealercard }
        } else if (dealervalue === yourvalue) {
            finalResult = { result: "TIE", method: "Hoà nhé", ycard: yourcard, dcard: dealercard }
        } else if (options.isDoubleDown === true) {
            if (yourvalue > 21) {
                finalResult = { result: "DOUBLE LOSE", method: "Bạn đã thua x2 tiền trong ván đấu này", ycard: yourcard, dcard: dealercard }
            } else if (yourvalue === 21) {
                finalResult = { result: "DOUBLE WIN", method: "Bạn đã được xì dách", ycard: yourcard, dcard: dealercard }
            } else if (yourvalue < 21 && dealervalue < yourvalue) {
                finalResult = { result: "DOUBLE WIN", method: "Bài của bạn thật tuyệt", ycard: yourcard, dcard: dealercard }
            } else if (dealervalue > yourvalue && dealervalue > 21 && yourvalue < 21) {
                finalResult = { result: "DOUBLE WIN", method: "Còn gì may mắn hơn khi được x2 không ?", ycard: yourcard, dcard: dealercard }
            } else if (dealervalue === 21 && yourvalue < 21) {
                finalResult = { result: "DOUBLE LOSE", method: "Nhà cái đã được xì dách", ycard: yourcard, dcard: dealercard }
            } else if (dealervalue > yourvalue && dealervalue < 21) {
                finalResult = { result: "DOUBLE LOSE", method: "Bài của nhà cái ngon hơn nyc của bạn =)))", ycard: yourcard, dcard: dealercard }
            } else if (dealervalue === yourvalue) {
                finalResult = { result: "DOUBLE TIE", method: "Hoà nhé", ycard: yourcard, dcard: dealercard }
            }
        }
